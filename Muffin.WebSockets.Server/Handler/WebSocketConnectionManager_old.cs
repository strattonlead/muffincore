...
namespace Muffin.WebSockets.Server.Handler
{
    ...
    {
        internal void InvokeOnSocketAdded(SocketInfo socketInfo, HttpContext httpContext)
        {
            if (socketInfo != null)
            {
                OnSocketAdded?.Invoke(socketInfo, httpContext);
            }
        }

        internal void InvokeOnSocketRemoved(SocketInfo socketInfo)
        {
            if (socketInfo != null)
            {
                OnSocketRemoved?.Invoke(socketInfo, null);
            }
        }
    ...
    public delegate void SocketEvent(SocketInfo socketInfo, HttpContext httpContext);
